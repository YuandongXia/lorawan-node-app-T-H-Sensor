...
//===========================================================================
{
    ...
    I2c_StartCondition();
    error |= I2c_WriteByte (I2C_ADR_R);
    I2C_SCL_INPUT();                     // set SCL I/O port as input
    for(i=0; i<1000; i++)         // wait until master hold is released or
    { DelayMs(1);    // a timeout (~1s) is reached
    if (I2C_SCL_IN()==1) break;
    }
    //-- check for timeout --
    if(I2C_SCL_IN()==0) error |= TIME_OUT_ERROR;
...
//===========================================================================
{
    ...
    DelayMs(15); // wait till sensor has restarted
