...
void update_rssi_snr(lw_sta_t sta)
{
    ...
    default:
        ...
        else if((app_lw_rx->snr != 0)||(app_lw_rx->rssi != 0)){
            rssi = (uint8_t)((int16_t)180+app_lw_rx->rssi);
            snr = app_lw_rx->snr;
            if(snr == -1){
                snr = 0;
    ...
    if(fmt != SHT_FMT_STEE){
        ...
        if(fmt == SHT_FMT_DIR){
          ...
          if(sht == SHT2X){
            ...
            tf = (((175.72 * temp) / 65536) - 46.85 ) * 100.0 ;
            // H = 125*h/2^16-6
            hf = (((125.0 * rh) / 65536) - 6.0) * 100.0 ;     
          }
          else if(sht == SHT3X){
            // T = t/(2^16-1)*175-45
            tf = ((175.0 * temp)/ 65535 - 45.0) * 100.0 ;
            //  H = h/(2^16-1)*100;
            hf = ((100.0 * rh) / 65535) * 100.0 ;
          }
/*
    Byte       1        1        1            2             2          2       1
    ITEM    Header    RSSI    Battery    Temperature    Humidity    Period    SNR
*/
          buf[0] = 0x43 ;
          buf[1] = rssi ;
          buf[2] = (vdd-150) & 0xFF;
          buf[3] = ((int16_t)tf>>0) & 0xFF;
          buf[4] = ((int16_t)tf>>8) & 0xFF;
          buf[5] = ((uint16_t)hf>>0) & 0xFF;
          buf[6] = ((uint16_t)hf>>8) & 0xFF;
          buf[7] = (period>>0) & 0xFF;
          buf[8] = (period>>8) & 0xFF;     
          buf[9] = snr;
          *len = 10 ;
...
ABP_CHECK_RET:
            if( ret == 0 ){
                ...
                if(ret == LORAMAC_STATUS_MAC_CMD_LENGTH_ERROR || ret == LORAMAC_STATUS_LENGTH_ERROR){
                    ret = lw_send(mcps, (uint8_t)cfg_read(CFG_PORT), NULL, 0, cfg_read(CFG_RETRY), -1);
                    goto ABP_CHECK_RET;
                }
            }
