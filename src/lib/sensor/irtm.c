...
void irtm_init(void)
{
    hal_adc_init(&irtm_obj_in, ADC_IN1);
    hal_adc_init(&irtm_env_in, ADC_IN);
    hal_adc_read(&irtm_obj_in);
    hal_adc_read(&irtm_env_in);
}
...
float irtm_obj_read()
{
    ...
    ret = (float)hal_adc_get_vdd()/100.0*hal_adc_read(&irtm_obj_in)/ADC_FULL_SCALE;
...
float irtm_sur_read()
{
    ...
    ret = (float)hal_adc_get_vdd()/100.0*hal_adc_read(&irtm_env_in)/ADC_FULL_SCALE;
...
float offset_vol=0.014;        //this parameter was used to set the mid level voltage,when put the sensor in normal environment after 10 min,
                               //the sensor output 0.For example,the surrounding temperature is 29¡æ£¬but the result is 27¡æ via the sensor,
...
/*1*/             { 0.271,0,-0.303,-0.642,-1.018,-1.434,-1.894,-2.398,-2.951,-3.556,-4.215,-4.931},  //¡úsurrounding temperature,from -10,0,10,...100
/*2*/             { 0.567,0.3,0,-0.335,-0.708,-1.121,-1.577,-2.078,-2.628,-3.229,-3.884,-4.597},   //¡ýobject temperature,from -10,0,10,...110
...
float arraysearch(float x,float y)//x is the surrounding temperature,y is the object temperature
{
	...
	i=(int)(x/10)+1;//Ambient temperature
...
//	temp1=objtValue*1.1/1023;//+objt_calibration;
	...
	array_temp=(unsigned int)arraysearch(current_temp,sur_temp*1000);
	temp2=current_temp;
