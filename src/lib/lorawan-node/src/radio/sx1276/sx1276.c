...
extern log_rf_t log_rf;
#define LOG_RF(x)                do { log_rf.flag = true; \
                                     log_rf.tx = x; \
                                     log_rf.freq = SX1276.Settings.Channel; \
                                     log_rf.sf = SX1276.Settings.LoRa.Datarate; \
                                     log_rf.bw = SX1276.Settings.LoRa.Bandwidth; }while(0)
#define LOG_RF_FSK(x)            do { log_rf.flag = true; \
                                     log_rf.tx = x; \
                                     log_rf.freq = SX1276.Settings.Channel; \
                                     log_rf.sf = SX1276.Settings.Fsk.Datarate; \
                                     log_rf.bw = SX1276.Settings.Fsk.Bandwidth; }while(0)
#define LOG_PUSH(type, buf, len) log_push(type, buf, len)
#else
#define LOG_RF(x)
#define LOG_RF_FSK(x)
#define LOG_PUSH(type, buf, len)
#endif
/*
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    SX1276SetRfTxPower( power, SX1276.Settings.Channel, true );
...
void SX1276OnTimeoutIrq( void )
{
    ...
    case RF_RX_RUNNING:
        ...
        }else if( SX1276.Settings.Modem == MODEM_LORA ){
    ...
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    LOG_PUSH(false, RxTxBuffer, SX1276.Settings.FskPacketHandler.Size);
            ...
            case MODEM_LORA:
                {
                    ...
                    if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )
                    {
                        LOG_PUSH(false, RxTxBuffer, SX1276.Settings.LoRaPacketHandler.Size);
