...
typedef struct
{
    uint16_t Subseconds;        //unit is various
    uint8_t Hours;
    uint8_t Minutes;
    uint8_t Seconds;
}TimeContext_t;
...
typedef struct RtcCalendar_s
{
    ...
    DateContext_t CalendarDate; //! Reference time in calendar format
    TimeContext_t CalendarTime; //! Reference date in calendar format
}RtcCalendar_t;
