...
{
  uint32_t idx = 0;
#if defined(STM32L1XX_MD) || defined(STM32L1XX_HD)|| defined(STM32L1XX_MD_PLUS) || defined(STM32L052xx)|| defined(STM32L053xx)
  ...
  while (malPointer < (uint32_t*)(MAL_Buffer + DataLength))
  {
    ...
    FlashProgramPage(((uint32_t)memPointer & 0xFFFFFF80), 128, (uint8_t *)(memBuffer));
#else
    /* Write the buffer to the memory*/
